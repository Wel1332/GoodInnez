...
import { 
  ...
  Bell, BriefcaseMedical, Flame 
} from 'lucide-react';

export default function AddProperty({ user: propUser }) {
  const navigate = useNavigate();
  
  // --- FAILSAFE: Load user from storage if prop is missing ---
  ...
  // --- STRICT CHECK: Only redirect if REALLY not logged in ---
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      data.append("description", `Type: ${formData.propertyType}. ${formData.description}. Amenities: ${formData.amenities.join(', ')}`);
      data.append("stars", 5);
      
      // Use the safe 'user' object
      if (user) {
  ...
  const getTypeIcon = (type) => {
    switch(type) {
        case 'Apartment': return <Building2 size={24} className="text-gray-700" />;
        case 'Flat': return <Home size={24} className="text-gray-700" />;
        case 'Room': return <BedDouble size={24} className="text-gray-700" />;
        case 'Villa': return <Palmtree size={24} className="text-gray-700" />;
  ...
  const getAmenityIcon = (item) => {
    switch(item) {
        ...
        case 'Washer': return <Shirt size={18} />;
        case 'AC': return <Wind size={18} />;
        case 'Pool': return <Waves size={18} />;
        case 'Smoke Alarm': return <Bell size={18} />;
        case 'First Aid': return <BriefcaseMedical size={18} />;
        case 'Fire Extinguisher': return <Flame size={18} />;
  ...
  if (!user || user.userType !== 'employee') {
    return (
        <div className="pt-32 text-center">
            ...
            <p>Please log in as a Partner to add properties.</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        {['Apartment', 'Flat', 'Room', 'Villa'].map(type => (
                            <div key={type} onClick={() => updateData('propertyType', type)} 
                                ...
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${formData.propertyType === type ? 'bg-gold/20' : 'bg-gray-100'}`}>
                                    {getTypeIcon(type)}
                                </div>
                             ...
                             return (
                                <div key={item} onClick={() => toggleItem(field, item)} className={`p-4 border rounded-xl cursor-pointer flex items-center justify-between transition-all hover:border-black ${isSelected ? 'border-black bg-gray-50' : 'border-gray-200'}`}>
                                    <div className="flex items-center gap-3">
                                        <span className={`text-gray-500 ${isSelected ? 'text-black' : ''}`}>
                                            {getAmenityIcon(item)}
                                        </span>
